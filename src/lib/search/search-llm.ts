...
- If web search WILL help (facts, stats, current events, prices, schedules, citations), set "useWebSearch": true and produce between 1 and ${count} concise queries (you can create 1, 2, or up to ${count} queries depending on what's needed).
- You have flexibility: create 1 query for simple/focused questions, 2-3 queries for broader topics that benefit from multiple angles.
...
- Always include a concise reason (<=12 words) for why you chose to run or skip web search.
...
- Do not include commentary or extra fields.`;
  ...
  const reasonText = formatQueryReason(parsed?.reason);
  const shouldUseWebSearch = typeof parsed?.useWebSearch === "boolean" ? parsed.useWebSearch : true;
  if (!shouldUseWebSearch) {
    return {
      queries: [],
      useWebSearch: false,
      reason: formatQueryReason(parsed?.reason, "Search not needed"),
    };
  }
  ...
  return {
    ...
    reason: reasonText || "Query writer generated search",
  };
}
...
- If web search WILL help, set useWebSearch=true and produce between 1 and ${count} concise queries (you can create 1, 2, or up to ${count} queries depending on what's needed).
- You have flexibility: create 1 query for simple/focused questions, 2-3 queries for broader topics that benefit from multiple angles.
...
- timeSensitive=true only when data older than ~24h is insufficient (live scores, breaking news, current traffic/stock/flight status, "right now" weather); otherwise false.
  - Keep reasons short (<=12 words) and provide one every time. No extra fields.`;
...
function formatQueryReason(raw?: unknown, fallback = "No reason provided"): string {
