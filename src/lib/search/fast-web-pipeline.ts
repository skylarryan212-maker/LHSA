...
type PipelineOptions = {
  ...
  onQueryStart?: (event: { query: string; queryIndex: number; totalQueries: number }) => void;
...
export async function runWebSearchPipeline(
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    // Notify about individual query start
    options.onQueryStart?.({ query, queryIndex: i, totalQueries: queries.length });
