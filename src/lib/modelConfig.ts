...
function buildConfigFromRouterDecision(
  ...
  if ((decision.model === "gpt-5-mini" || decision.model === "gpt-5-nano" || decision.model === "gpt-oss-20b") && finalEffort === "none") {
    finalEffort = "low";
  }
...
export function getModelAndReasoningConfig(
  ...
  if (isGrokFamily(resolvedFamily)) {
    const effort = reasoningEffortHint ?? "low";
    const useReasoning =
  ...
  // If auto-routing moved us onto Mini/Nano/OSS, ensure effort is valid (no 'none' for small models)
  if ((resolvedFamily === "gpt-5-mini" || resolvedFamily === "gpt-5-nano" || resolvedFamily === "gpt-oss-20b")) {
...
export const VALID_SPEED_MODES: SpeedMode[] = ["auto"];
...
export function getModelSettingsFromDisplayName(displayName: string): ModelSettings {
  ...
  if (displayName === "GPT 5 Nano") return { modelFamily: "gpt-5-nano", speedMode: "auto" };

  // GPT 5 Mini options
  if (displayName === "GPT 5 Mini Auto") return { modelFamily: "gpt-5-mini", speedMode: "auto" };
  if (displayName === "GPT 5 Mini") return { modelFamily: "gpt-5-mini", speedMode: "auto" };

  // GPT OSS 20b options
  if (displayName === "GPT OSS 20b Auto" || displayName === "GPT OSS 20B Auto") {
    return { modelFamily: "gpt-oss-20b", speedMode: "auto" };
  }
  if (displayName === "GPT OSS 20b" || displayName === "GPT OSS 20B") {
    return { modelFamily: "gpt-oss-20b", speedMode: "auto" };
  }

  // GPT 5.2 options (explicit)
  if (displayName === "GPT 5.2 Auto") return { modelFamily: "gpt-5.2", speedMode: "auto" };
  if (displayName === "GPT 5.2") return { modelFamily: "gpt-5.2", speedMode: "auto" };
