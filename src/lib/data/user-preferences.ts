...
export async function updateAllowDataForImprovement(allowDataForImprovement: boolean) {
  const supabase = await supabaseServer();
  const userId = await requireUserIdServer();
  const supabaseAny = supabase as any;
  ...
  if (existing) {
    const patch: UserPreferencesUpdate = ({ allow_data_for_improvement: allowDataForImprovement, updated_at } as unknown) as UserPreferencesUpdate;
    const { data, error } = await supabaseAny
      .from("user_preferences")
      .update(patch)
      .eq("user_id", userId)
      .select()
      .single();
    if (error) throw new Error(`Failed to update data improvement preference: ${error.message}`);
    return data;
  }
  ...
  const insert: UserPreferencesInsert = ({
    user_id: userId,
    accent_color: "white",
    allow_data_for_improvement: allowDataForImprovement,
  } as unknown) as UserPreferencesInsert;
  const { data, error } = await supabaseAny
    .from("user_preferences")
    .insert([insert])
    .select()
