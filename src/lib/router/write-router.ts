...
export type WriterRouterInput = {
  ...
  topics?: TopicStub[];
  currentTopic: { id: string | null; description: string | null; label?: string | null };
};
...
function fallbackDescriptionFromMessage(message: string): string | null {
  const clean = (message || "").replace(/\s+/g, " ").trim();
...
function extractHeadingTitle(text: string | null | undefined): string | null {
  if (!text) return null;
  const match = text.match(/^#{1,4}\s+(.+?)\s*$/m);
...
export async function runWriterRouter(
  ...
  const systemPrompt = `You decide topic metadata updates, artifacts, and memory/permanent instruction writes. Respond with ONE JSON object only.
...
Scope: ONLY write to the current topic. Do NOT create/update any other topics. additionalTopicWrites must stay empty.
{
  ...
  "additionalTopicWrites": [{ "action": "update" | "skip", "targetTopicId": "string|null", "label": "string|null", "description": "string|null" }],
...
Rules:
- Use action="create" only if topicAction=new; otherwise "update" only if you truly need to refresh description, else "skip".
- Never emit placeholder text like "none"/"null"/"n/a".
- label is only for create; set null for update/skip.
- Labeling goal: a broad, obvious topic title that matches the overall subject of the messages and helps the decision router select relevant context.
- Label format: 3-7 words, Title Case, noun-phrase; avoid questions and avoid filler like "Explain", "How to", "Question about".
- Prefer the assistant's first heading for the label; otherwise summarize the user request; fix obvious typos.
- Description: a simple, very brief sentence describing what is discussed in the topic.
...
- Artifacts: only emit artifacts that would help future turns on this topic. Base them on the assistant reply content, not the user message. Artifact content must summarize or highlight the assistant answer in concise form (ideally <400 characters) and must not reproduce the reply verbatim; if you cannot condense it, leave artifacts empty. Skip if the assistant reply is too short (<80 chars) or doesn't contain reusable material.
- Topic updates: only update the current topic; additionalTopicWrites must remain empty. Leave label/description null if no change.
- Arrays must be arrays (never null). No extra fields.`;
  ...
  const schema = {
    ...
    properties: {
      topicWrite: {
        ...
        required: ["action", "targetTopicId", "label", "description"],
      },
      memoriesToWrite: {
        type: "array",
        items: {
  ...
  const fallback = (): WriterRouterOutput => {
    ...
    const fallbackDescription = shouldCreate ? fallbackDescriptionFromMessage(input.userMessageText) : null;
    return {
      topicWrite: {
  ...
  try {
    const { text, usage, provider } = await callDeepInfraLlama({
      messages: [
    ...
    if (options?.userId && usage) {
      ...
      await logUsageRecord({
        ...
        estimatedCost,
        eventType: "router",
        metadata: { stage: "writer_router", provider },
      });
    }
    ...
    const label =
      action === "create"
        ? normalizeNullableText(topicWrite.label) ??
          extractHeadingTitle(input.assistantMessageText ?? "") ??
          autoLabelFromMessage(input.userMessageText)
        : null;
    const description =
      action === "create"
        ? normalizeNullableText(topicWrite.description) ?? fallbackDescriptionFromMessage(input.userMessageText)
