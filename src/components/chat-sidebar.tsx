...
import { KeyboardEvent, useEffect, useMemo, useRef, useState, memo } from 'react'
import { Button } from '@/components/ui/button'
import { ScrollArea } from '@/components/ui/scroll-area'
import { Plus, Sparkles, ChevronDown, ChevronRight, FolderPlus, X, Archive } from 'lucide-react'
...
interface ChatSidebarProps {
  ...
  claudeConversations?: Conversation[]
  projects?: Project[]
...
export const ChatSidebar = memo(function ChatSidebar({
  ...
  chatgptConversations: propChatgptConversations,
  claudeConversations: propClaudeConversations,
  ...
  const [claudeImportedProjectsCollapsed, setClaudeImportedProjectsCollapsed] = useState(false)
  const [chatsCollapsed, setChatsCollapsed] = useState(false)
  const [chatgptCollapsed, setChatgptCollapsed] = useState(false)
  ...
  const chatgptConversations: Conversation[] = useMemo(
    () => propChatgptConversations ?? [],
    [propChatgptConversations]
  )

  const claudeConversations: Conversation[] = useMemo(
    () => propClaudeConversations ?? [],
    [propClaudeConversations]
  )

  const projects: Project[] = propProjects || []
  // Imported projects are user projects created by the importer; we mark them
  // in the DB with metadata.imported = true. Show them in a separate section.
  const regularProjects = useMemo(() => projects.filter((p) => !Boolean((p.metadata as any)?.imported)), [projects])
  const importedProjects = useMemo(() => projects.filter((p) => Boolean((p.metadata as any)?.imported)), [projects])
  const claudeImportedProjects = useMemo(() => importedProjects.filter((p) => {
    const source = (p.metadata as any)?.source
    return source === 'claude' || !source
  }), [importedProjects])
  ...
  const visibleProjects = useMemo(() => regularProjects.slice(0, 5), [regularProjects])
  const moreProjects = useMemo(() => regularProjects.slice(5), [regularProjects])
  ...
  const primaryButtonClass = isOpen
    ? "w-full max-w-[240px] justify-start px-2.5 h-10 gap-2 text-sidebar-foreground hover:bg-sidebar-accent rounded-lg sidebar-entry overflow-hidden transition-all duration-200"
    : "w-12 h-10 p-0 justify-center gap-2 text-sidebar-foreground hover:bg-sidebar-accent rounded-lg sidebar-entry overflow-hidden transition-all duration-200";
  const listItemClass =
    "w-full max-w-[240px] justify-start px-2.5 py-2 gap-2 text-sidebar-foreground hover:bg-sidebar-accent rounded-lg sidebar-entry overflow-hidden transition-all duration-200";
  ...
  return (
    <>
      ...
      <div
        ...
        className={`
        sidebar-shell fixed lg:sticky lg:top-0 h-full bg-[#0f0f1a] z-50 lg:pt-2
      ...
      >
        <div className="flex h-full flex-col overflow-hidden">
          <div className={`flex h-[53px] items-center px-4 ${isOpen ? 'justify-between' : 'lg:justify-center justify-between'}`}>
            {isOpen && (
              <div className="text-lg font-bold gradient-text tracking-tight">
                Quarry
              </div>
            )}
            <Button 
              ...
              <ScrollArea className="h-full px-3 pb-20 touch-pan-y" viewportClassName="pr-2 touch-pan-y">
                <div className="space-y-4 py-3 pb-4">
                  <div>
                    ...
                    {!projectsCollapsed && (
                      <div className="space-y-1">
                        ...
                        {/* Regular Projects - shown first, directly under New project */}
                        ...
                        {/* Claude Imported Projects section */}
                        {claudeImportedProjects.length > 0 && (
                          <div className="mb-2 mt-3">
                            <button
                              onClick={() => setClaudeImportedProjectsCollapsed(!claudeImportedProjectsCollapsed)}
                              className="flex w-full items-center gap-1 mb-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
                            >
                              {claudeImportedProjectsCollapsed ? <ChevronRight className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                              IMPORTED FROM CLAUDE
                            </button>
                            {!claudeImportedProjectsCollapsed && claudeImportedProjects.map((project) => {
                              const chatsForProject = projectChatMap[project.id] || []
                              const visibleChats = chatsForProject.slice(0, 5)
                              const hasMoreChats = chatsForProject.length > 5
                              const isProjectActive =
                              ...
                              return (
                                <div key={`imported-${project.id}`} className="space-y-1">
                                  <Link
                                    href={`/projects/${project.id}`}
                                    onClick={(event) => {
                                      event.preventDefault()
                                    ...
                                    className={`group relative flex ${listItemClass} rounded-lg ${
                                      isProjectActive ? 'bg-zinc-800 text-white' : 'hover:bg-sidebar-accent'
                                    }`}
                                    data-sidebar-selected={isProjectActive ? "true" : "false"}
                                  >
                                    <div className="flex items-center gap-2 flex-1 min-w-0 pr-1">
                                      ...
                                      <span className="min-w-0 truncate text-sm text-sidebar-foreground">{project.name}</span>
                                    </div>
                                    <div className="flex-shrink-0">
                                      <ProjectContextMenu
                                        onRename={() => void queueRenameProject(project.id, project.name)}
                                    ...
                                    <div className="space-y-1">
                                      {visibleChats.map((chat) => (
                                        <div
                                          ...
                                          className={`group/chat flex w-full items-center rounded-lg pl-6 pr-2.5 py-1.5 text-left transition-colors sidebar-entry ${
                                            selectedChatId === chat.id ? 'bg-zinc-800 text-white' : 'hover:bg-sidebar-accent'
                                          }`}
                                          data-sidebar-selected={selectedChatId === chat.id ? 'true' : 'false'}
                                        >
                                          <div className="flex-1 min-w-0 pr-0">
                                            <AnimatedTitle chatId={chat.id} title={chat.title} className="block min-w-0 w-full truncate text-sm text-sidebar-foreground" />
                                          </div>
                                          <div className="flex-shrink-0 ml-2">
                                            <ChatContextMenu
                                              removeLabel={`Remove from ${project.name}`}
                        ...
                        {moreProjects.length > 0 && (
                              <div className="relative">
                                <button
                                  ...
                                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-sidebar-accent transition-colors min-w-0 overflow-hidden"
                                >
                  ...
                  {chatgptConversations.length > 0 && (
                    <div>
                      <button
                        onClick={() => setChatgptCollapsed(!chatgptCollapsed)}
                        className="flex w-full items-center gap-1 mb-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
                      >
                        {chatgptCollapsed ? <ChevronRight className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                        CHATGPT IMPORTED
                      </button>

                      {!chatgptCollapsed && (
                        <div className="space-y-1">
                          {chatgptConversations.map((conv) => (
                            <div
                              key={conv.id}
                              role="button"
                              tabIndex={0}
                  ...
                  {claudeConversations.length > 0 && (
                    <div>
                      <button
                        onClick={() => setClaudeCollapsed(!claudeCollapsed)}
                        className="flex w-full items-center gap-1 mb-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
                      >
                        {claudeCollapsed ? <ChevronRight className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                        CLAUDE IMPORTED
                      </button>

                      {!claudeCollapsed && (
                        <div className="space-y-1">
                          {claudeConversations.map((conv) => (
                            <div
                              key={conv.id}
                              role="button"
                              tabIndex={0}
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 border-t border-indigo-400/[0.12] bg-[#0f0f1a] p-3 pb-4 ${
