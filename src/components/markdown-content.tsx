...
const sanitizeCustomDotsTag = (text: string): string => {
  return text.replace(/<\/?dots\s*\/?>/gi, "...")
}
...
const CustomParagraph = (props: any) => {
  ...
  void node;
  return (
    <p className="text-base leading-relaxed text-foreground mb-4 break-words" {...rest}>
      {children}
    </p>
  );
};

export const MarkdownContent = memo(function MarkdownContent({ content, messageId, generatedFiles, citations, isStreaming }: MarkdownContentProps) {
  ...
  const normalizeIncompleteMarkdown = useCallback((text: string, isStreaming: boolean): string => {
    ...
    // Handle incomplete bold markers (**text without closing **)
    // Only fix if we're streaming and there's an unclosed **
    let result = text;
    
    // Count opening and closing ** markers
    const boldOpenMatches = result.match(/\*\*/g);
    const boldOpenCount = boldOpenMatches ? boldOpenMatches.length : 0;
    
    // If we have an odd number of ** markers, we have an incomplete bold
    if (boldOpenCount % 2 === 1) {
      // Find the last ** and check if it's at the end or followed by text without a closing **
      const lastBoldIndex = result.lastIndexOf('**');
      if (lastBoldIndex >= 0) {
        const afterBold = result.slice(lastBoldIndex + 2);
        // If there's text after the last ** and no closing **, temporarily close it for rendering
        if (afterBold.trim() && !afterBold.includes('**')) {
          // Add a temporary closing ** at the end (will be removed/replaced when streaming completes)
          result = result + '**';
        }
      }
    }
    
    // Handle incomplete italic markers (*text without closing *)
    // Only for single * (not **)
    const singleStarMatches = result.match(/(?<!\*)\*(?!\*)/g);
    const singleStarCount = singleStarMatches ? singleStarMatches.length : 0;
    
    if (singleStarCount % 2 === 1) {
      // Find the last single * that's not part of **
      let lastSingleStarIndex = -1;
      for (let i = result.length - 1; i >= 0; i--) {
        if (result[i] === '*' && (i === 0 || result[i - 1] !== '*') && (i === result.length - 1 || result[i + 1] !== '*')) {
          lastSingleStarIndex = i;
          break;
        }
      }
      if (lastSingleStarIndex >= 0) {
        const afterStar = result.slice(lastSingleStarIndex + 1);
        if (afterStar.trim() && !afterStar.match(/(?<!\*)\*(?!\*)/)) {
          result = result + '*';
        }
      }
    }
  ...
  const targetContent = isStreaming ? deferredContent : content;
  const safeContent = useMemo(
    () => {
      let text = targetContent || '';
      // First normalize incomplete markdown if streaming
      text = normalizeIncompleteMarkdown(text, isStreaming ?? false);
      // Then normalize math delimiters
      text = normalizeMathDelimiters(text);
      // Sanitize any literal <dots> tags that React/HTML don't recognize
      text = sanitizeCustomDotsTag(text);
      return text;
    },
    [targetContent, isStreaming, normalizeIncompleteMarkdown, normalizeMathDelimiters]
  );
  ...
  const InlineCitationBadge = ({ urls }: { urls?: string[] | string }) => {
    ...
    return (
      ...
      >
        ...
        <span className="pointer-events-none absolute left-1/2 bottom-full z-[60] w-72 -translate-x-1/2 translate-y-2 rounded-2xl border border-border bg-card/95 p-3 text-xs text-foreground opacity-0 transition duration-150 group-hover:opacity-100 group-hover:-translate-y-0 shadow-2xl">
          ...
          {domainLabel ? (
            <span className="mt-2 flex items-center gap-1 text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
  ...
  const TableRenderer = (props: any) => {
    ...
    useEffect(() => {
      const update = () => {
        ...
        // Force left alignment and top vertical alignment for all header and cell elements
        try {
          const cells = Array.from(tableEl.querySelectorAll('th, td')) as HTMLElement[]
    ...
    return (
      <div className="relative my-6 overflow-x-auto pr-12 hover:[&>button]:opacity-100">
        <button
          type="button"
          className="absolute right-2 inline-flex h-8 w-8 items-center justify-center rounded-md bg-transparent text-foreground/80 opacity-0 transition hover:text-foreground hover:bg-surface/30 border-0 shadow-none z-20 pointer-events-auto"
          ref={buttonRef}
          onClick={(e) => {
            e.stopPropagation()
        ...
        <table
          ref={tableRef}
          className="min-w-full border-collapse text-[14px] text-foreground/90 bg-surface/70 text-left [&_th]:text-left [&_th]:align-top [&_td]:text-left [&_td]:align-top"
          {...rest}
        />
      </div>
    )
  ...
  return (
    <div
      className="prose prose-invert max-w-none prose-pre:p-0 prose-pre:bg-transparent w-full max-w-full min-w-0 break-words prose-a:break-words prose-p:leading-relaxed prose-ol:ml-0 prose-ul:ml-0 prose-li:pl-0 [&_li>p]:my-0 [&_li>ul]:my-0 [&_li>ol]:my-0"
    >
      {markdownNode}
