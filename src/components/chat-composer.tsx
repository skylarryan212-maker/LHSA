...
import { getMaxFilesPerMessage } from "@/lib/usage-limits";
...
function getVoiceMessageFileName(mimeType: string) {
  ...
  const extension = AUDIO_MIME_TYPE_EXTENSIONS[normalized] ?? normalized.split("/")[1] ?? "webm";
  return `voice-message.${extension}`;
}
// Maximum textarea grow height (px) before internal scrolling (reduced by ~40%)
const MAX_TEXTAREA_HEIGHT = 192;
// We must not read `sessionStorage` during render (SSR). Instead,
// defer reading until the client layout phase to avoid hydration
// mismatches for attributes like `data-force-focus`.
...
export const ChatComposer = memo(function ChatComposer({
  ...
  shouldGrowDownward: shouldGrowDownwardProp,
  ...
  const restoreFocusOnMountRef = useRef<boolean>(false);
  ...
  // Determine grow direction: prefer explicit prop, otherwise auto-detect from pathname
  const [localGrowDownward, setLocalGrowDownward] = useState<boolean>(Boolean(shouldGrowDownwardProp ?? false));
  useEffect(() => {
    if (typeof shouldGrowDownwardProp === "boolean") {
      setLocalGrowDownward(shouldGrowDownwardProp);
      return;
    }
    if (typeof window === "undefined") return;
    const p = window.location.pathname || "/";
    const growDown = p === "/" || p.startsWith("/agents/human-writing") || p.startsWith("/human-writing");
    setLocalGrowDownward(growDown);
  }, [shouldGrowDownwardProp]);

  // Report composer height to parent if requested so outer containers can reserve space
  useEffect(() => {
    ...
    const obs = new ResizeObserver((entries) => {
      ...
      if (!entry) return;
      const height = (entry.contentRect && entry.contentRect.height) || (composerRef.current?.offsetHeight ?? 0);
      if (raf) cancelAnimationFrame(raf);
      raf = requestAnimationFrame(() => onHeightChange(height));
    });
    if (composerRef.current) obs.observe(composerRef.current);
    // initial report
    const initial = composerRef.current ? composerRef.current.offsetHeight : 0;
  ...
  useEffect(() => {
    const maxFiles = getMaxFilesPerMessage(plan);
    if (attachments.length <= maxFiles && attachmentError?.toLowerCase().includes("limit")) {
      setAttachmentError(null);
    }
  }, [attachments.length, attachmentError, plan]);
  ...
  // Read the restore-focus flag in a layout effect so the value is
  // available before paint on the client but not read during SSR.
  useLayoutEffect(() => {
    if (typeof window === "undefined") return;
    let shouldRestore = false;
    try {
      shouldRestore = window.sessionStorage.getItem(RESTORE_FOCUS_KEY) === "1";
    } catch {
      shouldRestore = false;
    }
    restoreFocusOnMountRef.current = shouldRestore;
    setForceFocus(shouldRestore);
    ...
    // Empty deps: run once on client layout
  }, []);
  ...
  const effectiveSubmit = async (text: string) => {
    ...
    if (attachments.length > maxFiles) {
      setAttachmentError(`File limit reached (${maxFiles} per message).`);
      return;
    }
  ...
  // Autosize textarea to fit content up to MAX_TEXTAREA_HEIGHT
  useLayoutEffect(() => {
    ...
    // Toggle internal scrolling when content exceeds max
    if (scrollH > MAX_TEXTAREA_HEIGHT) {
      ta.style.overflowY = "auto";
    } else {
      ta.style.overflowY = "hidden";
    }
    // If composerRef exists, trigger a resize via the ResizeObserver (it will pick up the new height)
    if (composerRef.current && typeof window !== "undefined") {
      // Force a layout tick so ResizeObserver reports updated size
      requestAnimationFrame(() => {
        // noop - ResizeObserver will fire automatically when layout changes
      });
    }
  }, [value, isTranscribing, attachments.length]);
  ...
  const handleFilesSelected = async (files: FileList | File[] | null) => {
    ...
    if (fileArray.length > remainingSlots) {
      setAttachmentError(`Only ${remainingSlots} more file${remainingSlots === 1 ? "" : "s"} allowed (limit ${maxFiles}).`);
    } else {
      setAttachmentError(null);
    }
    ...
    try {
      ...
      const fileReads = filesToAdd.map((file) => {
  ...
  return (
    <form onSubmit={handleFormSubmit}>
      ...
      <div
        ref={composerRef}
        data-force-focus={forceFocus ? "true" : "false"}
        suppressHydrationWarning
        className={`composer-shell relative z-10 ${stackedActions ? "flex flex-col items-stretch min-h-[112px] space-y-2 py-3" : `flex ${localGrowDownward ? "items-start" : "items-end"} gap-1.5 sm:gap-2 py-2 sm:py-2.5`} rounded-3xl border border-border bg-card px-2 sm:px-3 lg:px-4 transition-all focus-within:border-ring`}
      >
        {isRecording ? (
          ...
          <>
            <Textarea
              ...
              className="min-h-[40px] max-h-[192px] border-0 bg-transparent dark:bg-transparent px-0 py-2 text-base leading-5 resize-none focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none rounded-none overflow-y-auto"
            />
          ...
          <>
            ...
            <Textarea
              ...
              rows={1}
              disabled={isRecording || isTranscribing}
              className="flex-1 min-h-[40px] max-h-[192px] border-0 bg-transparent dark:bg-transparent px-0 py-2.5 text-base leading-5 resize-none focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none rounded-none overflow-y-auto"
            />
