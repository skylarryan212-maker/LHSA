...
export const ApiUsageBadge = memo(function ApiUsageBadge() {
  ...
  const loadData = useCallback(async (forceRefresh: boolean = false) => {
    try {
      const [monthlyTotal, plan] = await Promise.all([
        getMonthlySpending(forceRefresh),
        getUserPlan(),
      ]);
  ...
  return (
    ...
    >
      ...
      <span className="tabular-nums">${spending.toFixed(4)} / ${limit.toFixed(2)}</span>
      {restrictionLabel && <span className="text-[10px] font-semibold">{restrictionLabel}</span>}
    </div>
  );
});

ApiUsageBadge.displayName = 'ApiUsageBadge';
