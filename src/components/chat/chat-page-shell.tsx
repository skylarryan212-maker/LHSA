...
import { ChatMessage, MemoizedChatMessage } from "@/components/chat-message";
...
import { CompactionIndicator } from "@/components/chat/status-indicators";
...
import { getContextWindowLimit } from "@/lib/usage-limits";
...
type SearchStatusEvent =
  ...
  | { type: "search-query-start"; query: string; queryIndex: number; totalQueries: number }
...
type RuntimeIndicatorVariant = "default" | "extended" | "search" | "reading" | "analyzing" | "compacting" | "compaction-complete" | "error" | "warning";
...
// Don't assume a default plan-sized context window on first render -
// the server will populate accurate values via stream metadata.
const DEFAULT_CONTEXT_WINDOW_TOKENS: number | null = null;
...
// Use performance-optimized flush interval from perf config
const STREAM_UPDATE_INTERVAL_MS = getEffectiveFlushInterval();
...
// Use throttled search progress from perf config
const SEARCH_PROGRESS_THROTTLE_MS = PERF_CONFIG.SEARCH_PROGRESS_THROTTLE_MS;
...
function normalizeModelForSpeedMode(displayName: string): string {
  ...
  if (normalized.includes("oss") || normalized.includes("gpt-oss")) return "GPT OSS 20b";
...
function getSpeedModeModelSettings(displayName: string): ModelSettings {
  ...
  switch (normalizedDisplay) {
    ...
    case "GPT OSS 20b":
      return { modelFamily: "gpt-oss-20b", speedMode: "auto" };
    case "GPT 5 Mini":
      return { modelFamily: "gpt-5-mini", speedMode: "auto" };
    case "GPT 5.2":
      return { modelFamily: "gpt-5.2", speedMode: "auto" };
    case "GPT 5.2 Pro":
      return { modelFamily: "gpt-5.2-pro", speedMode: "auto" };
    default:
      return { modelFamily: "gpt-5-nano", speedMode: "auto" };
  }
}
...
export default function ChatPageShell({
  ...
  const selectedAgent = agentSelectionByChat[conversationKey] ?? null;
  const isAgentDarkTheme =
    selectedAgent === "market-agent" || selectedAgent === "sga" || selectedAgent === "human-writing";
  ...
  useLayoutEffect(() => {
    ...
    const computedSidebarColor = getComputedStyle(root).getPropertyValue("--color-sidebar").trim();
    const sidebarElement = document.querySelector(".sidebar-shell");
    const sidebarBackground = sidebarElement
      ? getComputedStyle(sidebarElement).backgroundColor
      : "";
    const resolvedBackground =
      sidebarBackground && sidebarBackground !== "rgba(0, 0, 0, 0)"
        ? sidebarBackground
        : computedSidebarColor || "#0f0f1a";
    root.dataset.chatThemeToken = token;
    root.style.backgroundColor = resolvedBackground;
    body.style.backgroundColor = resolvedBackground;
    return () => {
      if (typeof document === "undefined") return;
      if (root.dataset.chatThemeToken !== token) return;
      const path = window.location.pathname || "";
      const isChatRoute =
        path === "/" ||
        path.startsWith("/c/") ||
        /^\/projects\/[^/]+\/c\//.test(path);
      if (isChatRoute) return;
      delete root.dataset.chatThemeToken;
  ...
  useEffect(() => {
    return () => {
      if (newTopicTimerRef.current) {
        clearTimeout(newTopicTimerRef.current);
      }
      if (compactionCompleteTimerRef.current) {
        clearTimeout(compactionCompleteTimerRef.current);
      }
    };
  }, []);
  ...
  const [compactionCompleteMessage, setCompactionCompleteMessage] = useState(false);
  const compactionCompleteTimerRef = useRef<NodeJS.Timeout | null>(null);
  const [newTopicNotification, setNewTopicNotification] = useState<{ label: string; id: string } | null>(null);
  const newTopicTimerRef = useRef<NodeJS.Timeout | null>(null);
  const notifyTopicCreated = useCallback((topicData: { id: string; label: string }) => {
    if (!topicData?.label) return;
    setNewTopicNotification(topicData);
    if (newTopicTimerRef.current) {
      clearTimeout(newTopicTimerRef.current);
    }
    newTopicTimerRef.current = setTimeout(() => {
      console.log('[topic-notification] Auto-hiding notification');
      setNewTopicNotification(null);
      newTopicTimerRef.current = null;
    }, 4000);
  }, []);
  ...
  const pendingAutoScrollChatRef = useRef<string | null>(null);
    ...
    (status: SearchStatusEvent) => {
      switch (status.type) {
        case "search-start":
          ...
          break;
        case "search-query-start":
          // Update to show the specific query being searched
          setSearchIndicator((prev) => ({
            message: `Searching for "${status.query}"`,
            variant: "running",
            domains: prev?.domains ?? [],
            subtext: status.totalQueries > 1 ? `Query ${status.queryIndex + 1} of ${status.totalQueries}` : undefined,
          }));
          break;
        ...
        case "compaction-start":
          ...
          setCompactionCompleteMessage(false);
          if (compactionCompleteTimerRef.current) {
            clearTimeout(compactionCompleteTimerRef.current);
            compactionCompleteTimerRef.current = null;
          }
          break;
        case "compaction-complete":
          setIsCompacting(false);
          setCompactionCompleteMessage(true);
          if (compactionCompleteTimerRef.current) {
            clearTimeout(compactionCompleteTimerRef.current);
          }
          compactionCompleteTimerRef.current = setTimeout(() => {
            setCompactionCompleteMessage(false);
            compactionCompleteTimerRef.current = null;
          }, 4000); // 4 seconds
          break;
  ...
  // Cleanup compaction timer on unmount
  useEffect(() => {
    return () => {
      if (compactionCompleteTimerRef.current) {
        clearTimeout(compactionCompleteTimerRef.current);
  ...
  }, [activeIndicatorMessageId, searchIndicator, fileReadingIndicator, thinkingStatus, isAnalyzing, isStreaming]);
  ...
  useEffect(() => {
    if (selectedChatId) {
      pendingAutoScrollChatRef.current = selectedChatId;
    } else {
      pendingAutoScrollChatRef.current = null;
    }
  }, [selectedChatId]);

  useEffect(() => {
    const targetChatId = pendingAutoScrollChatRef.current;
    if (!targetChatId || targetChatId !== selectedChatId) return;
    if (showEmptyConversation) {
      pendingAutoScrollChatRef.current = null;
      return;
    }
    if (!hasPendingNewChat && renderedMessages.length === 0) return;
  ...
  const streamGuestResponse = async (
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        ...
        let ndjsonBuffer = "";
	      // RAF-aligned batching for guest streaming (same pattern as main streaming)
	      let guestTokenBuffer = "";
	      let guestMetadataBuffer: AssistantMessageMetadata | null = null;
	      let guestRafId: number | null = null;
	      let guestLastFlush = 0;

      flushGuestUpdate = () => {
        if (guestRafId) {
          cancelAnimationFrame(guestRafId);
          guestRafId = null;
        }
        if (!guestTokenBuffer) return;
	        updateMessage(chatId, assistantId, {
	          content: guestTokenBuffer,
	          ...(guestMetadataBuffer ? { metadata: guestMetadataBuffer } : {}),
	        });
	        guestTokenBuffer = "";
	        guestMetadataBuffer = null;
	      };
      ...
      const scheduleGuestUpdate = (
        ...
        guestTokenBuffer = content;
        guestMetadataBuffer = metadata;
        if (guestRafId) {
          return; // Already scheduled
        }
	        guestRafId = requestAnimationFrame(() => {
	          guestRafId = null;
	          const now = performance.now();
	          if (now - guestLastFlush < STREAM_UPDATE_INTERVAL_MS) {
	            // Re-schedule if we're within the throttle window
	            guestRafId = requestAnimationFrame(() => {
	              guestRafId = null;
	              if (!guestTokenBuffer) return;
	              updateMessage(chatId, assistantId, {
	              ...
	              perfCounters.recordUICommit();
	              guestTokenBuffer = "";
	              guestMetadataBuffer = null;
	              guestLastFlush = performance.now();
	            });
	            return;
	          }
	          if (!guestTokenBuffer) return;
            ...
            if (parsed.token) {
              ...
              scheduleGuestUpdate(assistantContent, messageMetadata);
                ...
                flushGuestUpdate();
	              ...
	              // Let finally block handle cleanup so animation can complete
	              return;
	            }
	          } catch (e) {
      ...
      // Delay cleanup to allow final content update to be processed with isStreaming=true
      setTimeout(() => {
  ...
  const handleSubmit = async (
    ...
    if (isCompacting) return; // Prevent sending during compaction
  ...
  const finalizeStreamingState = useCallback(
    () => {
      ...
      const streamChatId = activeStreamStateRef.current?.chatId ?? selectedChatId;
      if (streamChatId) {
        conversationUiStateRef.current[streamChatId] = {
          isStreaming: false,
          thinkingStatus: null,
          searchIndicator: null,
          fileReadingIndicator: null,
          activeIndicatorMessageId: null,
          responseTiming: { start: null, firstToken: null, assistantMessageId: null },
      ...
      // Dispatch usage update so UI reflects cost of completed turn
      if (typeof window !== "undefined") {
  ...
  const streamModelResponse = useCallback(async (
    ...
    // ─── Performance-optimized streaming state ───
    // Use refs for "hot" state (token buffer) to avoid React re-renders per token
    const tokenBufferRef = { current: '' };
    const metadataBufferRef = { current: messageMetadata };
    let updateTimer: ReturnType<typeof setTimeout> | null = null;
    let rafId: number | null = null;
    let lastFlushTime = 0;
    const flushInterval = STREAM_UPDATE_INTERVAL_MS;
    
    // Batched flush function - coalesces multiple tokens into single UI update
    const flushStreamUpdate = () => {
      perfMark('stream-flush');
      if (rafId !== null) {
      ...
      const metadata = metadataBufferRef.current;
      if (!content && !metadata) {
        perfMeasure('stream-flush');
        return;
      }
      
      const messageId = responseTimingRef.current.assistantMessageId ?? assistantMessageId;
      
      // Ensure chat exists in state before updating (important for new chats after navigation)
      // After navigation, the chat might not be in state yet, so ensure it exists
      let effectiveChatId = chatId;
      const chatExists = chats.find(c => c.id === chatId);
      if (!chatExists) {
        // Chat doesn't exist - ensure it's created (this can happen after navigation for new chats)
        ensureChat({
          id: chatId,
          title: "New chat",
          timestamp: new Date().toISOString(),
        ...
        // Fallback to selectedChatId if available, otherwise use chatId
        // The ensureChat call will add it to state, but React state updates are async
        effectiveChatId = selectedChatId && chats.find(c => c.id === selectedChatId) ? selectedChatId : chatId;
      }
      
      // Single UI commit for all buffered tokens
      perfCounters.recordUICommit();
    ...
    // Schedule a batched update using requestAnimationFrame for smooth rendering
    const scheduleStreamUpdate = () => {
      if (rafId !== null || updateTimer !== null) return;

      const elapsed = Date.now() - lastFlushTime;
      const remaining = Math.max(0, flushInterval - elapsed);
    ...
    try {
      // Ensure chat exists in state (important for new chats after navigation)
      // This prevents updateMessage/appendMessages from silently failing if chat doesn't exist yet
      const chatExists = chats.find(c => c.id === chatId);
      if (!chatExists) {
        // Chat doesn't exist - ensure it's created before streaming
        // This can happen when a new chat is created and navigation occurs
        ensureChat({
          id: chatId,
          title: "New chat",
          timestamp: new Date().toISOString(),
      ...
      if (!reader) {
        ...
        let ndjsonBuffer = "";
	      ...
	      try {
            ...
            while (true) {
              ...
              if (!line.trim()) continue;
                ...
                // Buffer tokens for batched UI update (performance optimization)
                tokenBufferRef.current = assistantContent;
                if (messageMetadata) {
                  metadataBufferRef.current = messageMetadata;
                }
                scheduleStreamUpdate();
                perfMeasure('token-process');
              ...
              } else if (parsed.topic_created) {
                const topicData = parsed.topic_created as { id: string; label: string };
                console.log('[topic-notification] Topic created event received:', topicData);
                notifyTopicCreated(topicData);
              } else if (parsed.type === "web_search_domain" && typeof parsed.domain === "string") {
                addSearchDomain(parsed.domain);
                // Update metadata buffer with accumulated domains (batched with tokens)
                const currentMessageId =
                  responseTimingRef.current.assistantMessageId ?? assistantMessageId;
                if (messageMetadata) {
                  const updatedMetadata: AssistantMessageMetadata = {
                  ...
                  messageMetadata = updatedMetadata;
                  metadataBufferRef.current = messageMetadata;
                  // Domain updates are "cold" - don't force immediate flush
                }
                ...
                if (messageMetadata) {
                  ...
                  messageMetadata = updatedMetadata;
                  metadataBufferRef.current = messageMetadata;
                  // Model info is "cold" metadata - schedule update
                  scheduleStreamUpdate();
                }
		              } else if (parsed.textDone) {
		                // Text generation finished, though backend tasks (artifacts etc) might still be running.
		                // Release the "Stop Generating" button and thinking indicators now.
		                finalizeStreamingState();
		              } else if (parsed.meta) {
		                sawMeta = true;
                ...
                // Handle new topic notification
                const meta = (parsed.meta as any);
                console.log('[topic-notification] Checking metadata:', {
                  hasMeta: !!meta,
                  hasMetadata: !!meta.metadata,
                  metadataKeys: meta.metadata ? Object.keys(meta.metadata) : [],
                  topicCreated: meta.metadata?.topicCreated,
                  fullMeta: JSON.stringify(meta, null, 2).substring(0, 500), // First 500 chars
                });
                // Check both locations: metadata.topicCreated and direct topicCreated
                const topicCreated = meta.metadata?.topicCreated || meta.topicCreated;
                if (topicCreated) {
                  const topicData = topicCreated as { id: string; label: string };
                  console.log('[topic-notification] Topic created detected:', topicData);
                  notifyTopicCreated(topicData);
                } else {
                  console.log('[topic-notification] No topicCreated in metadata');
                }
                ...
                if (chatId) {
                  const existingUi = conversationUiStateRef.current[chatId];
                  if (existingUi) {
                    conversationUiStateRef.current[chatId] = {
                      ...existingUi,
                      activeIndicatorMessageId: newId,
                      responseTiming: {
                        ...existingUi.responseTiming,
                ...
                updateMessage(chatId, assistantMessageId, {
                  ...
                  metadata: {
                    ...metadataWithTiming,
                    stableKey: assistantMessageId, // Keep original temp ID as a stable key for animations
                  },
                ...
                // Early release: once metadata arrives, we consider the generation phase "done" for the UI.
                // This allows the "Stop Generating" button to become "Send" immediately.
                finalizeStreamingState();
		              } else if (parsed.done) {
		                // Streaming complete - ensure final content is flushed immediately
		                sawDone = true;
                    // Ensure buffer has latest content before flushing
                    if (assistantContent) {
                      tokenBufferRef.current = assistantContent;
                    }
                    if (messageMetadata) {
                    ...
                    // Cancel any pending RAF/timer and flush synchronously to ensure UI updates
                    if (rafId !== null) {
                      cancelAnimationFrame(rafId);
                      rafId = null;
                    }
                    ...
                    // Flush final content immediately (synchronous, no RAF delay)
                    flushStreamUpdate();
                    // Emit usage update event for live counter at the absolute end too
                    if (typeof window !== 'undefined') {
                      window.dispatchEvent(new CustomEvent('api-usage-updated'));
                    }
                    finalizeStreamingState();
	      ...
	      // Cancel any pending batched updates and flush final state
	      if (rafId !== null) {
	        cancelAnimationFrame(rafId);
	        rafId = null;
	      }
  ...
  const handleRetryWithModel = async (retryModelName: string, messageId: string) => {
    ...
    if (retryModelName === "GPT OSS 20b" || retryModelName === "GPT OSS 20B") {
      retryModelFamily = "gpt-oss-20b";
      retrySpeedMode = "auto";
      retryReasoningOverride = speedModeEnabled ? "low" : undefined;
    } else if (retryModelName === "GPT 5 Nano") {
  ...
  const VirtuosoScroller = useMemo(() => {
    const Scroller = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
      ({ className, onScroll, onWheel, onTouchMove, onPointerDown, ...rest }, ref) => {
        const handleScrollPassive = useCallback((event: React.UIEvent<HTMLDivElement>) => {
          onScroll?.(event);
          handleScroll(event);
        }, [onScroll]);

        const handleTouchMovePassive = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
          onTouchMove?.(event);
        }, [onTouchMove]);
        ...
        return (
          <div
            ...
            ref={(node) => {
              ...
              // Add passive event listeners for scroll and touch
              if (node) {
                node.addEventListener('scroll', handleScrollPassive as any, { passive: true });
                node.addEventListener('touchmove', handleTouchMovePassive as any, { passive: true });
              }
            }}
            onWheel={(event) => {
              onWheel?.(event);
  ...
  const sidebarChatgptConversations = useMemo(
    () =>
      chatgptChats
        .map((chat) => ({
          id: chat.id,
  ...
  const sidebarClaudeConversations = useMemo(
    () =>
      claudeChats
        .map((chat) => ({
          id: chat.id,
  ...
  const runtimeIndicatorContent = useMemo<RuntimeIndicatorState | null>(() => {
    ...
    const hasIndicator = Boolean(thinkingStatus || searchIndicator || fileReadingIndicator || isAnalyzing || isCompacting || compactionCompleteMessage);
    const lastHasContent = messages.length === 0 || Boolean(messages[messages.length - 1]?.content);
    const allowRegardless = Boolean(searchIndicator || fileReadingIndicator || thinkingStatus || isAnalyzing || isCompacting || compactionCompleteMessage);

    if (!hasIndicator || (!lastHasContent && !allowRegardless)) {
      return null;
    }

    // Compaction indicators take precedence when active
    if (isCompacting) {
      return { 
  ...
  const assistantShimmerRgb = useMemo(() => {
    if (isCompacting) return "255, 165, 60"; // orange for compacting
    if (compactionCompleteMessage) return "60, 220, 140"; // green for complete
    ...
    return "255, 255, 255";
  }, [compactionCompleteMessage, fileReadingIndicator, isAnalyzing, isCompacting, searchIndicator, thinkingStatus]);

  const shouldRenderRuntimeIndicatorSlot =
    Boolean(selectedChatId && lastUserMessageId) &&
    (reserveRuntimeIndicatorSpace ||
      Boolean(thinkingStatus || searchIndicator || fileReadingIndicator || isAnalyzing || isCompacting || compactionCompleteMessage));

  const chatSurfaceClass = isAgentDarkTheme ? "bg-black" : "bg-[#151527]";
  const chatShellBackgroundClass = isAgentDarkTheme ? "bg-black" : "bg-[#0f0f1a]";
  const chatBorderClass = isAgentDarkTheme ? "border-border/60" : "border-indigo-400/[0.08]";
  const chatGradientClass = !isAgentDarkTheme && showEmptyConversation ? "copilot-gradient-bg show-gradient" : "";
  ...
  const initialMessageIndex = chatListItems.length > 0 ? chatListItems.length - 1 : 0;
  ...
  return (
    <div className={`${chatShellBackgroundClass} flex h-[100dvh] max-h-[100dvh] w-full min-w-0 text-foreground dark overflow-hidden overscroll-y-none is-chat-page`}>
      {/* Sidebar */}
      {!isGuest && (
        <ChatSidebar
          ...
          chatgptConversations={sidebarChatgptConversations}
          claudeConversations={sidebarClaudeConversations}
      ...
      {/* Right column: header + messages + composer - Floating Card */}
      <div
        ref={mainPanelRef}
        data-main-panel="true"
        className={`${chatSurfaceClass} flex flex-1 flex-col w-full min-w-0 min-h-0 overflow-hidden lg:my-2 lg:mr-2 lg:rounded-2xl lg:border ${chatBorderClass} ${chatGradientClass}`}
        style={{ viewTransitionName: "main-panel", ["--assistant-shimmer-rgb" as any]: assistantShimmerRgb }}
      >
        {/* Header bar */}
        <div className={`sticky top-0 z-20 flex h-[53px] items-center justify-between border-b ${chatBorderClass} ${chatSurfaceClass} px-3 lg:px-6 lg:rounded-t-2xl`}>
                  ...
                  >
                    {speedModeEnabled ? (
                      <>
                        ...
                        <DropdownMenuItem
                          className="items-center gap-3 px-3 py-2"
                          onSelect={() => setCurrentModel("GPT OSS 20b")}
                        >
                          <div className="flex flex-1 flex-col">
                            <span className="font-medium leading-none">GPT OSS 20b</span>
                            <span className="text-xs text-muted-foreground">Low cost, open-weight</span>
                          </div>
                          <span className="flex w-4 justify-end">
                            {effectiveModelDisplay === "GPT OSS 20b" && <Check className="h-4 w-4" />}
                          </span>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          className="items-center gap-3 px-3 py-2"
                      ...
                      <>
                        ...
                        >
                          <div className="flex flex-1 flex-col">
                            <span className="font-medium leading-none">GPT OSS 20b</span>
                            <span className="text-xs text-muted-foreground">Open-weight, low cost</span>
                          </div>
                          <span className="flex w-4 justify-end">
                            {(currentModel === "GPT OSS 20b" || currentModel === "GPT OSS 20B") && (
          ...
          <div className="flex items-center gap-3">
            {!isGuest && (
              <ContextUsageIndicator
                ...
                setIsCompacting={setIsCompacting}
                compactionCompleteMessage={compactionCompleteMessage}
                setCompactionCompleteMessage={setCompactionCompleteMessage}
                compactionCompleteTimerRef={compactionCompleteTimerRef}
                newTopicNotification={newTopicNotification}
                setNewTopicNotification={setNewTopicNotification}
              />
            )}
            {isGuest ? (
              <>
        ...
        >
          {showEmptyConversation ? (
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              <Virtuoso
                key={`${conversationKey}-pending`}
                ref={virtuosoRef}
                style={{ height: "100%" }}
                data={pendingNewChatMessages ?? []}
                initialTopMostItemIndex={
                  pendingNewChatMessages?.length ? pendingNewChatMessages.length - 1 : 0
                }
                computeItemKey={(index, message) => (message.metadata as any)?.stableKey ?? message.id ?? String(index)}
                  ...
                  <div className="px-4 sm:px-6">
                    <div className="w-full max-w-[min(48rem,calc(100vw-32px))] min-w-0 mx-auto px-1.5 sm:px-0 overflow-hidden">
                      <ChatMessage
                        ...
                        stableKey={(message.metadata as any)?.stableKey || message.id}
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              <Virtuoso
                ...
                computeItemKey={(index, message) => {
                  if (isRuntimeIndicatorSlotMessage(message)) return message.id;
                  return (message.metadata as any)?.stableKey ?? message.id ?? String(index);
                }}
                itemContent={(index, message) => {
                  ...
                  // Only show global insight chips (thinking time, search domains) for completed messages
                  // This prevents duplicate model tags/thinking indicators while the global runtime indicator is active
                  ...
                  const messageStableId = (message.metadata as any)?.stableKey || message.id;

                  return (
                    <React.Fragment key={`${messageStableId}-${index}`}>
                      <div>
      ...
      {PERF_CONFIG.DEV_PERF_MODE && <PerfOverlay />}
    </div>
  );
}
...
function ContextUsageIndicator({
  ...
  setIsCompacting,
  compactionCompleteMessage,
  setCompactionCompleteMessage,
  compactionCompleteTimerRef,
  newTopicNotification,
  setNewTopicNotification,
}: {
  ...
  compactionCompleteMessage: boolean;
  setCompactionCompleteMessage: React.Dispatch<React.SetStateAction<boolean>>;
  compactionCompleteTimerRef: React.MutableRefObject<NodeJS.Timeout | null>;
  newTopicNotification: { label: string; id: string } | null;
  setNewTopicNotification: React.Dispatch<React.SetStateAction<{ label: string; id: string } | null>>;
}) {
  ...
  useEffect(() => {
    console.log('[topic-notification] State changed:', newTopicNotification);
  }, [newTopicNotification]);

  // Ensure we have a fresh plan value on mount so the estimate reflects
  // any recent upgrades/changes to the user's plan.
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  // Determine an effective token limit: prefer server-provided `usage.limit`,
  // otherwise fall back to the user's plan context window.
  const effectiveLimit =
    typeof usage.limit === "number" && Number.isFinite(usage.limit)
      ? usage.limit
      : getContextWindowLimit(plan || "free");
  ...
  const limitTokens = Math.max(0, effectiveLimit || 0);
  const formatTopicNotice = (label: string) =>
    label === "New Topic" ? "New Topic" : `New topic: ${label}`;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-[3px] rounded-full bg-background context-window-inner" />
      </div>
      <span className="text-sm font-semibold text-foreground">{percent}%</span>
      {newTopicNotification ? (
        <div className="absolute right-0 top-[calc(100%+6px)] z-40 pointer-events-none">
          <div className="flex max-w-[220px] items-center gap-2 rounded-full border border-border/80 bg-card/95 px-3 py-1 text-xs font-medium text-foreground shadow-lg backdrop-blur-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-sky-400" />
            <span className="truncate">{formatTopicNotice(newTopicNotification.label)}</span>
          </div>
        </div>
      ) : null}

      {isOpen ? (
        <div className="absolute right-0 top-[115%] z-30 w-64 rounded-lg border border-border/80 bg-card/95 text-foreground shadow-xl backdrop-blur-sm context-window-panel">
          <div className="relative space-y-1.5 p-3">
            ...
            <div className="text-xs text-muted-foreground">
              {typeof usage.limit === "number" && Number.isFinite(usage.limit) ? (
                ...
                <>{formatTokenCount(0)} / {formatTokenCount(limitTokens)} tokens used</>
              )}
            </div>
            <div className="pt-2">
              ...
              <div className="mt-2 flex items-center gap-2">
                {activeChatId ? (
                  <Button
                    size="sm"
                    variant="outline"
                    className={`h-7 px-2 text-xs ${isCompacting ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={isCompacting}
                    onClick={async () => {
                      if (!activeChatId || isCompacting) return;
                      
                      // Determine what to compact based on context mode
                      const compactScope = contextMode === "advanced" && advancedTopicIds && advancedTopicIds.length > 0
                        ? `the ${advancedTopicIds.length} selected topic(s)`
                        : "all topics in this conversation";
                      
                      if (!confirm(`Compact ${compactScope}? This will summarize older messages to free up space while preserving key information.`)) return;
                      ...
                      try {
                        // In advanced mode with specific topics selected, compact only those topics
                        // In simple mode or advanced mode with "all" selected, compact all topics
                        const requestBody = contextMode === "advanced" && advancedTopicIds && advancedTopicIds.length > 0
                          ? { topicId: advancedTopicIds[0] } // For now, compact the first/primary topic
                          : { conversationId: activeChatId }; // Compact all topics
                        
                        console.log(`[compaction] Starting compaction in ${contextMode} mode:`, requestBody);
                        
                        const res = await fetch('/api/compaction/force', {
                          method: 'POST',
                          headers: { 'content-type': 'application/json' },
                        ...
                        if (!res.ok) {
                          setIsCompacting(false);
                          alert('Compaction failed: ' + (payload?.error || res.statusText || res.status));
                        } else {
                          const messagesCompacted = payload?.totalMessagesCompacted ?? 0;
                          const topicsProcessed = payload?.topicsProcessed ?? 0;
                          const skippedReasons = payload?.skippedReasons;
                          
                          console.log(`[compaction] ✅ Completed: ${messagesCompacted} messages compacted across ${topicsProcessed} topics`);
                          
                          if (messagesCompacted === 0 && skippedReasons) {
                            console.warn(`[compaction] ⚠️ No messages were compacted. Reasons:`, skippedReasons);
                            alert(`Compaction completed but no messages were compacted.\n\nReasons:\n${Object.entries(skippedReasons).map(([reason, count]) => `• ${count} topic(s): ${reason}`).join('\n')}\n\nCheck the server console for more details.`);
                          } else {
                            // Show success
                            setCompactionCompleteMessage(true);
                        ...
                        setIsCompacting(false);
                        alert('Compaction request failed');
                      }
                    }}
                  >
                ...
                {compactionCompleteMessage && (
                  <span className="text-[11px] text-green-500">✓ Compacted</span>
                )}
                {newTopicNotification && (
                  <span className="text-[11px] text-blue-400 font-medium">
                    {formatTopicNotice(newTopicNotification.label)}
                  </span>
                )}
              </div>
            </div>
